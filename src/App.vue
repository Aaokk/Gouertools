<template>
  <template v-if="standaloneLayout">
    <router-view />
  </template>
  <template v-else>
      ...
      <aside class="sidebar">
        <div class="sidebar-brand">
          <div class="brand-sub">
            ...
            <div>
              <h2>GOUER</h2>
              <span>{{ $t('sidebar.brandSubtitle', { version: displayedVersion }) }}</span>
            </div>
          </div>
        </div>
        <nav class="sidebar-nav" :aria-label="$t('sidebar.navLabel')">
          ...
          <router-link to="/mosaic" class="nav-item" active-class="active">
            ...
            <span>{{ $t('sidebar.mosaic') }}</span>
          </router-link>
          <router-link to="/text-counter" class="nav-item" active-class="active">
            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M4 6h16M4 12h16M4 18h7"/></svg>
            <span>{{ $t('sidebar.textCounter') }}</span>
          </router-link>
          <router-link to="/app-icon" class="nav-item" active-class="active">
            ...
            <span>{{ $t('sidebar.appIcon') }}</span>
          </router-link>
        </nav>
        <div class="sidebar-footer">
          {{ $t('sidebar.copyright') }}<br>
...
export default {
  ...
  computed: {
    /** 不含侧栏的全屏路由（样式由页面自身独立 CSS） */
    standaloneLayout () {
      return this.$route.meta?.standaloneLayout === true
    },
    brandLogoHref () {
