      ...
      <div class="main-container">
        ...
        <div class="control-panel">
          ...
          <div class="settings-group">
            ...
            <div class="setting-item" v-if="originalFile && originalFile.type === 'image/png'">
              <label>目标大小:</label>
              <div style="flex: 1; display: flex; align-items: center; gap: 8px;">
                <input
                    type="number"
                    v-model="settings.pngCompression.maxSizeMB"
                    min="0.1"
                    max="10"
                    step="0.1"
                >
            ...
            <div class="setting-item" v-if="originalFile && originalFile.type === 'image/png'">
              ...
              <div style="flex: 1; display: flex; align-items: center; gap: 8px;">
                <input
                    type="number"
                    v-model="settings.pngCompression.maxWidthOrHeight"
                    min="800"
                    max="4000"
                    step="100"
                >
            ...
            <div class="setting-tip" v-if="originalFile && originalFile.type === 'image/png'">
              提示：调整目标大小和最大尺寸可以有效压缩PNG图片，同时保持较好的图片质量。
            </div>
          </div>
        </div>
      </div>
...
const processImage = async (file) => {
  try {
    ...
    const options = {
      maxSizeMB: settings.pngCompression.maxSizeMB,
      maxWidthOrHeight: settings.pngCompression.maxWidthOrHeight,
      useWebWorker: true,
      fileType: file.type
    }
    ...
    if (file.type === 'image/png') {
      ...
      options.maxSizeMB = file.size / (1024 * 1024) * settings.quality
    }
...
const resetSettings = () => {
  ...
  settings.pngCompression.maxSizeMB = 1
  settings.pngCompression.maxWidthOrHeight = 1920
  if (originalFile.value) {
    handleCompress()
  }
}
